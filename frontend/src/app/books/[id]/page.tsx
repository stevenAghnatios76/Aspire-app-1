  ...
  const fetchBook = useCallback(async () => {
    try {
      ...
      const active = history.find(
        (r) => r.book_id === id && (r.status === "pending" || r.status === "active" || r.status === "overdue" || r.status === "pending_return")
      );
  ...
  if (error || !book) {
    return (
      <div className="max-w-4xl mx-auto px-6 py-12 text-center">
        ...
        <Link href="/books" className="text-indigo-600 hover:underline text-sm mt-4 inline-block">
          ← Back to catalog
        </Link>
      </div>
    );
      ...
      <div className="flex flex-col md:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="flex flex-wrap gap-3 mb-6">
            <span className={`text-sm px-3 py-1 rounded-full font-medium ${statusStyles[book.status]}`}>
          ...
          <div className="flex flex-col gap-3">
            ...
            {borrowSuccess && (
              <p className="text-sm text-green-600">Checkout requested! Awaiting librarian approval.</p>
            )}
            {borrowError && (
